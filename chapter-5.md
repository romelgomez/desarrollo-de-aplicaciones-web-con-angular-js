...
El valor de user.admin es establecido a true si el checkbox es comprobado y false de lo contrario. Por el contrario, el checkbox será marcado si el valor de user.admin es verdadero.
...
Los botones de radio proporcionan un grupo fijo de opciones para un campo. AngularJS
hace esto realmente simple de implementar: Solo une todos los botones radio
en un grupo para el mismo campo de modelo. El atributo HTML estándar value es entonces
usado para especificar que valor colocar en el modelo cuando el radio es seleccionado:
...
Seleccione el correo electrónico de un usuario
...
Cada directiva ngModel crea una instancia de Controlador ngModel. Este controlador está a disposición de todas las directivas en el elemento input.
