...
Siempre que pases datos sobre HTTP entre un cliente y un servidor, hay una oportunidad para que terceros espien en la información segura, o incluso peor, acceder a sus cookies para secuestrar su sección y acceder al servidor, como si fuera usted.  Esto se denomina a menudo un ataque “hombre-en-el-medio”, consulte:
http://es.wikipedia.org/wiki/Ataque_Man-in-the-middle. La forma más fácil de prevenir estos ataques es usar HTTPS en lugar de HTTP.
...
Hay una vulnerabilidad de inyección de JSON que permite a sitios web malignos de terceros acceder a sus recursos JSON seguros, si se retornan matrices JSON. Esto es realizado efectivamente cargando el JSON como un script en una página web, y después ejecutarlo. Consulta:
...
NOTA DEL TRADUCDOR: Al momento de traducir esta sección se detecto que el enlace esta roto, ahora esta funcionalidad está a cargo de otro servicio http://angular-ui.github.io/bootstrap/#/modal. Se invita a revisar los otros servicios que angular-ui ofrece.

En nuestro servicio security, tenemos dos ayudantes: openLoginDialog() y
closeLoginDialog() .
